...
public class Lesson3_7 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_7";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(307).getViewed());
        db.updateLesson(new DBLesson(307, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(307).getViewed());
        ...
        TextView textView = findViewById(R.id.lesson3_7TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_7ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=IPv6&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>DHCP|<li>IPv6)(.*?)</ul>|<span id=\\\\\"((?!Stateless_address_autoconfiguration_.28SLAAC.29|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        while (matcher.find()){
                            ...
                            s = s.replaceAll("<link(.*?)>", "");
                            s = s.replaceAll("<sup>", "^");
                            s = s.replaceAll("<p>|</p>|<b>|</b>|<i>|</i>|<ul>|</ul>|<li>|</li>|<ol>|</ol>|<span>|</span>|</sup>|\\\"|>", "");
                            content.add(s);
                        }
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_7.this,android.R.layout.simple_list_item_1,content){
                            @Override
