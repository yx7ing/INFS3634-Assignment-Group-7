...
public class Lesson3_4 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_4";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(304).getViewed());
        db.updateLesson(new DBLesson(304, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(304).getViewed());
        ...
        setContentView(R.layout.activity_lesson3_4);
        ...
        TextView textView = findViewById(R.id.lesson3_4TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_4ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Network_layer&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(?!The following|Functions of)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li><cite|<li>OSI)(.*?)</ul>|<span id=\\\\\"((?!Relation_to_TCP.2FIP_model|Protocols|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_4.this,android.R.layout.simple_list_item_1,content){
                            @Override
