...
public class Lesson3_12 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_12";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(312).getViewed());
        db.updateLesson(new DBLesson(312, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(312).getViewed());
        ...
        setContentView(R.layout.activity_lesson3_12);
        ...
        TextView textView = findViewById(R.id.lesson3_12TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_12ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Internet_Control_Message_Protocol&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(?!Where)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>RFC|<li>IANA)(.*?)</ul>|<span id=\\\\\"((?!RFCs|header_|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_12.this,android.R.layout.simple_list_item_1,content){
                            @Override
