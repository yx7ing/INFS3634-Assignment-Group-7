...
public class Lesson2_7 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson2_7";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(207).getViewed());
        db.updateLesson(new DBLesson(207, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(207).getViewed());
        ...
        setContentView(R.layout.activity_lesson2_7);
        ...
        TextView textView = findViewById(R.id.lesson2_7TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson2_7ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Data_link_layer&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li><cite|<li>D|<li>A)(.*?)</ul>|<span id=\\\\\"((?!Relation_to_the_TCP.2FIP_model|Protocol_examples|Examples|See_also|References|External_links).*?)\\\\\">");
                        Matcher matcher = pattern.matcher(response);
                        ArrayList<String> content = new ArrayList<>();
                        content.add("\nData Link Layer - MAC Layer and LLC Layer");
                        while (matcher.find()){
                            ...
                            s = s.replaceAll("_|\\\\n", " ");
                            s = s.replaceAll("\\\\\"", "\"");
                            s = s.replaceAll("<p>|</p>|<b>|</b>|<i>|</i>|<ul>|</ul>|<li>|</li>|<ol>|</ol>|<sub>|</sub>|<span>|</span>|\\\\u00a0|\\\"|>", "");
                            content.add(s);
                        }
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson2_7.this,android.R.layout.simple_list_item_1,content){
                            @Override
