...
public class Lesson4_9 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson4_9";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(409).getViewed());
        db.updateLesson(new DBLesson(409, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(409).getViewed());
        ...
        setContentView(R.layout.activity_lesson4_9);
        ...
        TextView textView = findViewById(R.id.lesson4_9TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson4_9ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Domain_Name_System&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        response = response.substring(0, 40608);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ol>(.*?)</ol>|<ul>(.*?)</ul>|<span id=\\\\\"((?!The_domain_name_space|Privacy.2Ftracking_issues|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        content.remove(88);
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson4_9.this,android.R.layout.simple_list_item_1,content){
                            @Override
