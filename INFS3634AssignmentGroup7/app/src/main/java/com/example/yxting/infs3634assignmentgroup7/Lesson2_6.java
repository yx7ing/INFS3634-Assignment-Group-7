...
public class Lesson2_6 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson2_6";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(206).getViewed());
        db.updateLesson(new DBLesson(206, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(206).getViewed());
        ...
        setContentView(R.layout.activity_lesson2_6);
        ...
        TextView textView = findViewById(R.id.lesson2_6TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson2_6ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Network_topology&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(?!<span><span><math)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>F)(.*?)</ul>|<span id=\\\\\"((?!Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson2_6.this,android.R.layout.simple_list_item_1,content){
                            @Override
