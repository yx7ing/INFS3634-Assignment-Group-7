...
public class Lesson1_3 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson1_3";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(103).getViewed());
        db.updateLesson(new DBLesson(103, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(103).getViewed());
        ...
        TextView textView = findViewById(R.id.lesson1_3TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson1_3ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Client%E2%80%93server_model&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ul>(?!<li>Mi)(.*?)</ul>|<span id=\\\\\"((?!Notes|See_also|References|External_links).*?)\\\\\">");
                        Matcher matcher = pattern.matcher(response);
                        ArrayList<String> content = new ArrayList<>();
                        content.add("\nNetworking Architecture - Client Server Model");
                        while (matcher.find()){
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson1_3.this,android.R.layout.simple_list_item_1,content){
                            @Override
