...
public class QuizCombined extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "QuizCombined";

    // Instantiate database
    DBHelper db = new DBHelper(this);

    // An int array to store 12 random question numbers
    final int[] questions = generateUniqueRandom();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz_combined);
        ...
        TextView textView = findViewById(R.id.quizCTextView);
        ...
        qna[0] = "Question " + questionNum[0] + ". " + db.getQuestion(questions[questionNum[0]-1]).getQuestion() + "\n";
        qna[1] = db.getQuestion(questions[questionNum[0]-1]).getAnswera() + "\n";
        qna[2] = db.getQuestion(questions[questionNum[0]-1]).getAnswerb() + "\n";
        qna[3] = db.getQuestion(questions[questionNum[0]-1]).getAnswerc() + "\n";
        qna[4] = db.getQuestion(questions[questionNum[0]-1]).getAnswerd() + "\n";
        final int[] answers = new int[12];
        ListView listView = (ListView) findViewById(R.id.quizCListView);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position > 0) {
                    ...
                    if (questionNum[0] < 12) {
                        ...
                        Intent i = new Intent(QuizCombined.this, QuizCombinedComplete.class);
    ...
    // Randomly generate 12 unique random numbers between 1 and 60
    // and return them in an int array
    private int[] generateUniqueRandom(){
        int[] nums = new int[12];
        Boolean[] notUnique = new Boolean[60];
        Arrays.fill(notUnique, Boolean.FALSE);
        for (int i = 0; i < 12; i++) {
            Boolean unique = false;
            while (unique == false){
                int temp = (int)(Math.floor(Math.random()*60))+1;
                if (notUnique[temp-1] == false) {
                    nums[i] = temp;
                    unique = true;
                }
            }
