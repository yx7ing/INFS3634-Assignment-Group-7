...
public class Quiz2Complete extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Quiz2Complete";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz2_complete);
        ...
        TextView textView = findViewById(R.id.quiz2cTextView);
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            if (answers[i] == db.getQuestion(i+13).getCorrectans()) {
                marks++;
                if (db.getReview(i+13).getCorrectonce() == 0) {
                    points += 75;
                }
                db.updateReview(new DBReview(i+13, answers[i], 1));
            } else {
                db.updateReview(new DBReview(i+13, answers[i], db.getReview(i+13).getCorrectonce()));
            }
        }
        if (marks == 12 && db.getQuiz(2).getCompleted() == 0){
            points += 100;
        }
        ...
        if (db.getQuiz(2).getCompleted() == 0){
            db.updateQuiz(new DBQuiz(2, 1, marks));
            db.updateTrophy(new DBTrophy(22, 1));
            Toast.makeText(this, "You got a trophy for completing Quiz 2!", Toast.LENGTH_LONG).show();
        } else {
            if (db.getQuiz(2).getHighest() < marks) {
                db.updateQuiz(new DBQuiz(2, 1, marks));
            }
        }
        Log.e(TAG, "Quiz Trophy 2 status is " + db.getTrophy(22).getEarned());

        if (marks == 12 && db.getTrophy(32).getEarned() == 0) {
            db.updateTrophy(new DBTrophy(32, 1));
            Toast.makeText(this, "You got a trophy for a perfect mark in Quiz 2!", Toast.LENGTH_LONG).show();
        }
        Log.e(TAG, "Full Marks Trophy 2 status is " + db.getTrophy(32).getEarned());

        // Set and format result textview
        TextView result = (TextView) findViewById(R.id.quiz2cTextView2);
