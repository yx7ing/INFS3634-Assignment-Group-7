...
public class Quiz3Complete extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Quiz3Complete";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz3_complete);
        ...
        TextView textView = findViewById(R.id.quiz3cTextView);
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            if (answers[i] == db.getQuestion(i+25).getCorrectans()) {
                marks++;
                if (db.getReview(i+25).getCorrectonce() == 0) {
                    points += 75;
                }
                db.updateReview(new DBReview(i+25, answers[i], 1));
            } else {
                db.updateReview(new DBReview(i+25, answers[i], db.getReview(i+25).getCorrectonce()));
            }
        }
        if (marks == 12 && db.getQuiz(3).getCompleted() == 0){
            points += 100;
        }
        ...
        if (db.getQuiz(3).getCompleted() == 0){
            db.updateQuiz(new DBQuiz(3, 1, marks));
            db.updateTrophy(new DBTrophy(23, 1));
            Toast.makeText(this, "You got a trophy for completing Quiz 3!", Toast.LENGTH_LONG).show();
        } else {
            if (db.getQuiz(3).getHighest() < marks) {
                db.updateQuiz(new DBQuiz(3, 1, marks));
            }
        }
        Log.e(TAG, "Quiz Trophy 3 status is " + db.getTrophy(23).getEarned());

        if (marks == 12 && db.getTrophy(33).getEarned() == 0) {
            db.updateTrophy(new DBTrophy(33, 1));
            Toast.makeText(this, "You got a trophy for a perfect mark in Quiz 3!", Toast.LENGTH_LONG).show();
        }
        Log.e(TAG, "Full Marks Trophy 3 status is " + db.getTrophy(33).getEarned());

        // Set and format result textview
        TextView result = (TextView) findViewById(R.id.quiz3cTextView2);
