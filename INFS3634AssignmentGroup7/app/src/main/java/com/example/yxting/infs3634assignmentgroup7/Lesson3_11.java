...
public class Lesson3_11 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_11";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(311).getViewed());
        db.updateLesson(new DBLesson(311, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(311).getViewed());
        ...
        setContentView(R.layout.activity_lesson3_11);
        ...
        TextView textView = findViewById(R.id.lesson3_11TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_11ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Address_Resolution_Protocol&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        response = response.substring(0, 13598);
                        Pattern pattern = Pattern.compile("<p>(?!<span)(.*?)</p>|<ol>(.*?)</ol>|<ul>(.*?)</ul>|<span id=\\\\\"((?!INARP|Standards_documents|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        while (matcher.find()){
                            ...
                            s = s.replaceAll("_|\\\\n|\\\\u00a0", " ");
                            s = s.replaceAll("\\\\\"", "\"");
                            s = s.replaceAll("<link rel(.*?)>", "");
                            s = s.replaceAll("<p>|</p>|<b>|</b>|<i>|</i>|<ul>|</ul>|<li>|</li>|<ol>|</ol>|\\\"|>", "");
                            content.add(s);
                        }
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_11.this,android.R.layout.simple_list_item_1,content){
                            @Override
