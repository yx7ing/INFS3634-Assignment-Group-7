...
public class Quiz4Complete extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Quiz4Complete";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz4_complete);
        ...
        TextView textView = findViewById(R.id.quiz4cTextView);
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            if (answers[i] == db.getQuestion(i+37).getCorrectans()) {
                marks++;
                if (db.getReview(i+37).getCorrectonce() == 0) {
                    points += 75;
                }
                db.updateReview(new DBReview(i+37, answers[i], 1));
            } else {
                db.updateReview(new DBReview(i+37, answers[i], db.getReview(i+37).getCorrectonce()));
            }
        }
        if (marks == 12 && db.getQuiz(4).getCompleted() == 0){
            points += 100;
        }
        ...
        if (db.getQuiz(4).getCompleted() == 0){
            db.updateQuiz(new DBQuiz(4, 1, marks));
            db.updateTrophy(new DBTrophy(24, 1));
            Toast.makeText(this, "You got a trophy for completing Quiz 4!", Toast.LENGTH_LONG).show();
        } else {
            if (db.getQuiz(4).getHighest() < marks) {
                db.updateQuiz(new DBQuiz(4, 1, marks));
            }
        }
        Log.e(TAG, "Quiz Trophy 4 status is " + db.getTrophy(24).getEarned());

        if (marks == 12 && db.getTrophy(34).getEarned() == 0) {
            db.updateTrophy(new DBTrophy(34, 1));
            Toast.makeText(this, "You got a trophy for a perfect mark in Quiz 4!", Toast.LENGTH_LONG).show();
        }
        Log.e(TAG, "Full Marks Trophy 4 status is " + db.getTrophy(34).getEarned());

        // Set and format result textview
        TextView result = (TextView) findViewById(R.id.quiz4cTextView2);
