...
public class Lesson3_9 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_9";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(309).getViewed());
        db.updateLesson(new DBLesson(309, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(309).getViewed());
        ...
        setContentView(R.layout.activity_lesson3_9);
        ...
        TextView textView = findViewById(R.id.lesson3_9TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_9ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Dynamic_Host_Configuration_Protocol&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(?!<br>)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>RFC)(.*?)</ul>|<span id=\\\\\"((?!Documented_elsewhere|IETF_standards_documents|Notes|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        while (matcher.find()){
                            ...
                            s = s.replaceAll("_|\\\\n", " ");
                            s = s.replaceAll("\\\\\"", "\"");
                            s = s.replaceAll("<abbr title=(.*?)>WPAD</abbr>", "WPAD");
                            s = s.replaceAll("<abbr>EAPoDHCP</abbr>", "EAPoDHCP");
                            s = s.replaceAll("<p>|</p>|<b>|</b>|<i>|</i>|<ul>|</ul>|<li>|</li>|<ol>|</ol>|<code>|</code>|\\\"|>", "");
                            content.add(s);
                        }
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_9.this,android.R.layout.simple_list_item_1,content){
                            @Override
