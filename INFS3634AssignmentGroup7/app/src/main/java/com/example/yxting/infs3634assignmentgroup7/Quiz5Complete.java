...
public class Quiz5Complete extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Quiz5Complete";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz5_complete);
        ...
        TextView textView = findViewById(R.id.quiz5cTextView);
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            if (answers[i] == db.getQuestion(i+49).getCorrectans()) {
                marks++;
                if (db.getReview(i+49).getCorrectonce() == 0) {
                    points += 75;
                }
                db.updateReview(new DBReview(i+49, answers[i], 1));
            } else {
                db.updateReview(new DBReview(i+49, answers[i], db.getReview(i+49).getCorrectonce()));
            }
        }
        if (marks == 12 && db.getQuiz(5).getCompleted() == 0){
            points += 100;
        }
        ...
        if (db.getQuiz(5).getCompleted() == 0){
            db.updateQuiz(new DBQuiz(5, 1, marks));
            db.updateTrophy(new DBTrophy(25, 1));
            Toast.makeText(this, "You got a trophy for completing Quiz 5!", Toast.LENGTH_LONG).show();
        } else {
            if (db.getQuiz(5).getHighest() < marks) {
                db.updateQuiz(new DBQuiz(5, 1, marks));
            }
        }
        Log.e(TAG, "Quiz Trophy 5 status is " + db.getTrophy(25).getEarned());

        if (marks == 12 && db.getTrophy(35).getEarned() == 0) {
            db.updateTrophy(new DBTrophy(35, 1));
            Toast.makeText(this, "You got a trophy for a perfect mark in Quiz 5!", Toast.LENGTH_LONG).show();
        }
        Log.e(TAG, "Full Marks Trophy 5 status is " + db.getTrophy(35).getEarned());

        // Set and format result textview
        TextView result = (TextView) findViewById(R.id.quiz5cTextView2);
