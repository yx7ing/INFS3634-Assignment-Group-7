...
public class Lesson2_4 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson2_4";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(204).getViewed());
        db.updateLesson(new DBLesson(204, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(204).getViewed());
        ...
        setContentView(R.layout.activity_lesson2_4);
        ...
        TextView textView = findViewById(R.id.lesson2_4TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson2_4ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Coaxial_cable&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        response = response.substring(0, 11990);
                        Pattern pattern = Pattern.compile("<p>(?!The c)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>T|<li>B|<li><i>RF)(.*?)</ul>|<span id=\\\\\"((?!Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson2_4.this,android.R.layout.simple_list_item_1,content){
                            @Override
