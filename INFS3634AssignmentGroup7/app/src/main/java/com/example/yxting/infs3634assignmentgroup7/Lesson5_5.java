...
public class Lesson5_5 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson5_5";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(505).getViewed());
        db.updateLesson(new DBLesson(505, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(505).getViewed());
        ...
        setContentView(R.layout.activity_lesson5_5);
        ...
        TextView textView = findViewById(R.id.lesson5_5TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson5_5ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Simple_Mail_Transfer_Protocol&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        response = response.substring(0, 29081);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ol>(.*?)</ol>|<ul>(.*?)</ul>|<span id=\\\\\"((?!Hop|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        content.remove(18);
                        content.remove(20);
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson5_5.this,android.R.layout.simple_list_item_1,content){
                            @Override
