...
public class Lesson5_2 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson5_2";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(502).getViewed());
        db.updateLesson(new DBLesson(502, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(502).getViewed());
        ...
        TextView textView = findViewById(R.id.lesson5_2TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson5_2ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Transmission_Control_Protocol&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        response = response.substring(0, 58190);
                        Pattern pattern = Pattern.compile("<p>(?!Whenever)(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>0 \\(8)(.*?)</ul>|<span id=\\\\\"((?!Timeout-based_retransmission|checksum_offload|CONNECTION-ESTABLISHMENT|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        while (matcher.find()){
                            ...
                            s = s.replaceAll("_|\\\\n|\\\\ua00a0", " ");
                            s = s.replaceAll("\\\\\"", "\"");
                            s = s.replaceAll("<p>|</p>|<b>|</b>|<i>|</i>|<ul>|</ul>|<li>|</li>|<ol>|</ol>|<tt>|</tt>|<code>|</code>|\\\"|>", "");
                            content.add(s);
                        }
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson5_2.this,android.R.layout.simple_list_item_1,content){
                            @Override
