...
public class Lesson1_6 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson1_6";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(106).getViewed());
        db.updateLesson(new DBLesson(106, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(106).getViewed());
        ...
        setContentView(R.layout.activity_lesson1_6);
        ...
        TextView textView = findViewById(R.id.lesson1_6TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson1_6ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Data_transmission&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ul>(?!<li>Mi)(.*?)</ul>|<span id=\\\\\"((?!Types_of_communication_channels_.28Network_topologies.29|Notes|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson1_6.this,android.R.layout.simple_list_item_1,content){
                            @Override
