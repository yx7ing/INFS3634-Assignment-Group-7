...
public class Lesson3_10 extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    private static final String TAG = "Lesson3_10";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.e(TAG, TAG + " view status is " + db.getLesson(310).getViewed());
        db.updateLesson(new DBLesson(310, 1));
        Log.e(TAG, TAG + " view status is " + db.getLesson(310).getViewed());
        ...
        TextView textView = findViewById(R.id.lesson3_10TextView);
        textView.setTextColor(Color.parseColor("#FF9933"));
        textView.setTypeface(textView.getTypeface(), Typeface.BOLD);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP,30);
        final ListView listView = findViewById(R.id.lesson3_10ListView);
        RequestQueue queue = Volley.newRequestQueue(this);
        String url = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&titles=Network_address_translation&exlimit=1";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Log.e(TAG, response);
                        Pattern pattern = Pattern.compile("<p>(.*?)</p>|<ol>(.*?)</ol>|<ul>(?!<li>Int|<li>NAT)(.*?)</ul>|<span id=\\\\\"((?!Notes|MASQUERADING|PORT-TRANSLATION|Type_of_NAT_and_NAT_traversal.2C_role_of_port_preservation_for_TCP|PORT-PRESERVATION|NAT_and_TCP.2FUDP|NAT_loopback|Examples|See_also|References|External_links).*?)\\\\\">");
                        ...
                        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(Lesson3_10.this,android.R.layout.simple_list_item_1,content){
                            @Override
